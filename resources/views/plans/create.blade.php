<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Kreiranje novog plana upravljanja otpadom') }}
        </h2>
    </x-slot>
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('plans.store') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="business_activity" class="block mt-1 w-full" type="text" name="business_activity" :value="old('business_activity')" required />
                                        ...
                                        <div class="flex items-center justify-between gap-4">
                                            ...
                                            <div class="flex items-center gap-3 flex-1 max-w-xs">
                                                <input type="number" id="quantity_{{ $key }}" name="waste_quantities[{{ $key }}]" 
                                                       value="{{ old('waste_quantities.'.$key, '0') }}" 
                                                       min="0" step="1"
                            ...
                            <div class="mt-2 flex space-x-6">
                                <div class="flex items-center">
                                    <input id="has_contract_yes" type="radio" name="has_contract_with_operator" value="1" 
                                           ...
                                           {{ old('has_contract_with_operator') == '1' ? 'checked' : '' }}>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="notes" name="notes" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" rows="4" placeholder="Opišite dodatne detalje o vašoj firmi, specifičnim potrebama za upravljanje otpadom, itd...">{{ old('notes') }}</textarea>
                            <x-input-error class="mt-2" :messages="$errors->get('notes')" />
                        </div>

                        <!-- Submit Button -->
                        <div class="flex items-center justify-end">
                            <a href="{{ route('plans.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded mr-4">
                                Otkaži
                            </a>
                            <x-primary-button>
                                Kreiraj plan
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
                ...
                wasteTypeCheckboxes.forEach(checkbox => {
                    ...
                    if (quantityContainer && quantityInput) {
                        if (checkbox.checked) {
                            ...
                            // Enable the input and focus if it's empty
            ...
            // Initial state - show inputs for pre-checked items (from form validation errors)
            setTimeout(function() {
